describe('core.pr', function()
  ...
  describe('plan_merge', function()
    ...
    it('allows a clean, open, mergeable pull request', function()
      local decision = pr.plan_merge(clean())
    ...
    it('refuses one that is already merged', function()
      local decision = pr.plan_merge(clean({ merged = true }))
    ...
    it('refuses one that is closed', function()
      local decision = pr.plan_merge(clean({ state = 'closed' }))
    ...
    it('refuses a conflicting branch and says so', function()
      local decision = pr.plan_merge(clean({ mergeable_state = 'dirty', mergeable = false }))
    ...
    it('refuses when a required review or check is missing', function()
      local decision = pr.plan_merge(clean({ mergeable_state = 'blocked' }))
    ...
    it('refuses a draft', function()
      assert.is_false(pr.plan_merge(clean({ mergeable_state = 'draft' })).allowed)
    end)

    it('refuses a branch that is behind its base', function()
      assert.is_false(pr.plan_merge(clean({ mergeable_state = 'behind' })).allowed)
    end)

    it('refuses when GitHub reports it as not mergeable', function()
      local decision = pr.plan_merge(clean({ mergeable = false, mergeable_state = 'unknown' }))
    ...
    it('refuses while GitHub is still computing mergeability', function()
      ...
      local decision = pr.plan_merge(pull)
    ...
    it('allows an unstable pull request but warns about the checks', function()
      local decision = pr.plan_merge(clean({ mergeable_state = 'unstable' }))
    ...
    it('allows one whose repository has hooks', function()
      assert.is_true(pr.plan_merge(clean({ mergeable_state = 'has_hooks' })).allowed)
    end)
  end)
end)
